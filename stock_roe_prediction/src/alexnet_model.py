...
class alexnet_model(cnn_model):
    ...
    def __init__(self, model_para, out_para, task_type=None):
        cnn_model.__init__(self, model_para, out_para, task_type)
        self.model_path = '{mp}/nn/alexnet/alexnet'.format(mp=out_para.get('model_path'))
        self.init_net()
    ...
    def init_net(self):
        ...
        self.weights = {
            ...
            'out': tf.Variable(tf.truncated_normal([1024, self.num_classes], dtype=tf.float32, stddev=0.1))
        }
        ...
        self.biases = {
            ...
            'out': tf.Variable(tf.constant(0.0, shape=[self.num_classes], dtype=tf.float32), trainable=True)
        }
